...
public:
    ...
    DynamicArray(int n, int min, int max){
        // set n to size
        size = n;
        // allocate memeory for size integers woth of memory
        array = new int[size];
        // set the value of ech integer in array to a random integer between min and max, both inclusive
        srand(time(0));
        for(int i = 0; i < size; i++) array[i] = GenerateRandom(min,max);
    }
 ...
 */
    ...
    void Sort(int index1, int index2){
        ...
        //sort array
        this->QuickSort(arrCopy, 0, newSize - 1);
        ...
        this->Add(index1,arrCopy,newSize);
